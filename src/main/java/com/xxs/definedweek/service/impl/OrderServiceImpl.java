...
public class OrderServiceImpl extends BaseServiceImpl<Order, Long> implements
	...
	public List<Order> findList(Member member, Integer count,
			List<Filter> filters, List<com.xxs.definedweek.Order> orders) {
	...
	public Page<Order> findPage(OrderStatus orderStatus,
			PaymentStatus paymentStatus, Boolean hasExpired, Pageable pageable) {
		return orderDao.findPage(orderStatus, paymentStatus, hasExpired,
				pageable);
	}
	...
	public Long count(OrderStatus orderStatus, PaymentStatus paymentStatus,
			Boolean hasExpired) {
		return orderDao.count(orderStatus, paymentStatus, hasExpired);
	}
	...
	@Transactional(readOnly = true)
	public Order build(Cart cart, PaymentMethod paymentMethod,
			...
			if (!couponCode.getIsUsed() && couponCode.getCoupon() != null
				...
				BigDecimal couponDiscount = cart.getEffectivePrice().subtract(
						couponCode.getCoupon().calculatePrice(
								cart.getQuantity(), cart.getEffectivePrice()));
	...
	public Order create(Cart cart, PaymentMethod paymentMethod,
			CouponCode couponCode, boolean isInvoice, String invoiceTitle,
		...
		Order order = build(cart, paymentMethod, couponCode, isInvoice,
	...
	public void complete(Order order, Admin operator) {
		...
		if (!member.getMemberRank().getIsSpecial()) {
			MemberRank memberRank = memberRankDao.findByAmount(member
					.getAmount());
			if (memberRank != null
			...
			if (orderItem != null) {
				...
				if (product != null) {
					...
					Calendar weekSalesCalendar = DateUtils.toCalendar(product
							.getWeekSalesDate());
					Calendar monthSalesCalendar = DateUtils.toCalendar(product
							.getMonthSalesDate());
